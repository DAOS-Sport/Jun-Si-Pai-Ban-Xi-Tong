...
- **Core Features**:
    ...
    - **Salary Hours Report**: Admin page at `/salary-report` showing per-employee monthly shift hours grouped by role type (e.g. 救生, 教練, 櫃台). Month picker, region filter, sort options. Shows KPI cards (total persons, total hours, avg hours, shift count). Table includes work hours per role, leave days per type, and totals. Exports to CSV. Backend `GET /api/salary-report?year&month&regionCode`.
