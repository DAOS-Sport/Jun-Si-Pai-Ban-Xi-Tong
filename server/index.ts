...
if (process.env.NODE_ENV === "production" && !process.env.SESSION_SECRET) {
  console.warn(
    "[WARN] SESSION_SECRET is not set. Using insecure fallback secret in production!",
  );
}
...
ensureSessionTable().catch((err) =>
  console.error("[session] 建立 session 表失敗:", err),
);
  ...
  httpServer.listen(
    ...
    () => {
      ...
      if (process.env.REPLIT_DEPLOYMENT !== "1") {
        log(
          "開發環境：已略過所有排程工作（避免 dev DB 資料不完整導致誤發 LINE 通知給員工）",
          "cron",
        );
      } else {
        cron.schedule(
          ...
          async () => {
            ...
            try {
              ...
              if (syncResult.errors.length > 0) {
                log(
                  `同步錯誤: ${syncResult.errors.slice(0, 10).join("; ")}`,
                  "cron",
                );
              }
            } catch (err: any) {
        ...
        cron.schedule(
          ...
          async () => {
            ...
            try {
              ...
              log(
                `班表推撥完成: 發送 ${result.sent}, 跳過 ${result.skipped}, 無LINE ${result.noLineId}`,
                "cron",
              );
            } catch (err: any) {
        ...
        cron.schedule(
          ...
          async () => {
            ...
            try {
              ...
              log(
                `未打卡檢查完成: 通知 ${result.notified}, 跳過 ${result.skipped}`,
                "cron",
              );
            } catch (err: any) {
        ...
        cron.schedule(
          ...
          async () => {
            ...
            try {
              ...
              log(
                `週班表推播完成: 發送 ${result.sent}, 跳過 ${result.skipped}, 無LINE ${result.noLineId}`,
                "cron",
              );
            } catch (err: any) {
        ...
        cron.schedule(
          ...
          async () => {
            ...
            try {
              ...
              log(
                `週遲到報告推播完成: 發送 ${result.sent}, 跳過 ${result.skipped}, 無LINE ${result.noLineId}`,
                "cron",
              );
            } catch (err: any) {
